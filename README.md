...
EnhancedInput사용, 조작감 향상을 위한 선입력 시스템 구현했습니다.

`AGTPlayerCharacter::OnAbilityInputActionPressed`

## 캐릭터 이동 및 카메라 제어

캐릭터 상태 (걷기, 달리기, 락온중) 에 따라 이동 속도, 캐릭터 회전, 카메라 제어를 관리합니다.
...
데미지를 가할때 가드, 패링 가능 유무 및 피격자 상태를 파악해 막기 결과를 도출합니다.

결과에 따라서 이펙트, 히트리액션, 데미지, 그로기 수치등 일련의 처리를 구현했습니다.
...
노티파이를 활용하여 애니메이션에 기반한 게임플레이를 제작했습니다.

`Source/GASToy/Animation`

## AI

BehaviorTree를 사용해 보스전투를 제작했습니다.

근접공격, 대쉬공격, 원거리 공격등을 거리기반으로 적절하게 구성해 다양한 전투경험을 하도록 구현했습니다.
...
어빌리티 시작시 몽타주를 재생하고 공격 이벤트를 수신받아 처리합니다.
...
공격 애니메이션은 크게 3단계로 구성되어 있습니다.

준비 → 공격 → 복귀

공격하는 구간에 선입력을 저장하고 공격이 끝나고 복귀하는 시점에 선입력을 실행해 다음 공격으로 진행합니다.

복귀 시점에 공격 종료 상태로 만들어 애니메이션이 완전히 종료 되기전 다음 액션으로 넘어갈 수 있게 처리했습니다.

어빌리티마다 태그를 부여하고 ActivationRequiredTags를 설정해 실행조건을 판단합니다.
...
유저의 입력 방향으로 구르고, 무적 상태를 부여합니다. 

8개의 방향 몽타주중 입력 방향에 가장 적합한 몽타주를 재생합니다.

ApplyRootMotionTask를 사용하여 캐릭터를 이동시킵니다.
...
가드를 하자마자 막은 경우는 패링을 합니다.

패링 가능 시간이 지난후 막으면 가드를 합니다.

막은 방향에 맞게 성공 애니메이션을 재생합니다.


가드 입력을 빠르게 반복하여 계속 패링하는 것을 방지하기 위해 쿨다운을 적용했습니다.

패링을 했다면 재입력시 바로 패링가능 상태로 만듭니다.
...
그로기 수치가 최대가 되면 그로기 상태가 되고, 이때 조작과 다른 어빌리티 실행을 할 수 없습니다.


스캔 그로기 어빌리티가 주기적으로 그로기 액터를 찾습니다.

찾았을 때 공격 버튼을 누르면 처형을 시작합니다.


처형 어빌리티가 그로기 액터를 이벤트로 넘겨 받아 처형시킵니다.

### 1.6 히트 리액션

`Source/GASToy/AbilitySystem/Ability/HitReaction`

히트 리액션을 기능별로 하나의 객체로 만들어, 다양한 조합이 가능하도록 제작했습니다.
...
각 어빌리티는 이벤트태그에 해당하는 데미지 데이터를 가집니다. / `TMap<FGameplayTag, FGTDamageData> EventTagToDamageData`

데미지 적용 이벤트를 받으면 데미지를 타깃에게 적용합니다.


데미지 적용은 일련의 처리를 합니다.

- 막기 검사를 하고 결과에 따라 게임플레이 이펙트(데미지, 그로기)적용, 게임플레이큐(VFX, SFX), 히트리액션, 가드와 패링을 처리합니다.
- 막기 결과를 조건으로 사용해 적절한 처리를 할 수 있게 구현했습니다.

## 3. 캐릭터 이동 및 카메라 제어

`UGTMovementControlComponent`

걷기, 달리기, 락온 상태등 캐릭터의 현재 상태에 따라 이동방식 및 이동속도를 다르게 처리했습니다.


특히 락온시 타깃과 가까울 때 카메라 회전이 빈번해 어지러움을 유발했고

이때 타깃과 가까울 수록 카메라 회전 보간 속도를 줄여 어지러움을 해결했습니다.

## 4. 선입력 시스템

`AGTPlayerCharacter::OnAbilityInputActionPressed`

액션 도중 다음 입력을 미리 저장해 조작감을 향상했습니다.

입력 데이터에 선입력 저장 여부를 설정하고

선입력을 사용하지 않으면 입력을 받자마자 실행합니다.

선입력을 사용하면 상황에 따라 다르게 처리합니다.
...
정밀한 충돌검사를 통해 게임플레이를 향상했습니다.

기존 오버랩 이벤트는 ImpactLocation을 구할 수 없고, 따라서 트레이스 검사를 사용했습니다.

무기의 모양에 맞춰 트레이스 구체를 연속적으로 설정해 정밀한 충돌검사를 수행했습니다.

`UGTCollisionSubsystem`

충돌 이벤트를 관리합니다.

중복 충돌 방지 : 어떤 스킬에서 발사체를 동시에 발사하고, 여러개를 맞더라도 하나의 충돌 이벤트만 발생 시키기 위해 범위와 아이디를 설정해 구현했습니다.

## 6. 애니메이션

`Source/GASToy/Animation`

블렌드스페이스, 몽타주, 루트모션, 애님 노티파이를 활용했습니다.
...
노티파이를 배치하여 애니메이션 기반한 게임플레이를 구현했습니다.

루트모션

기본 루트모션을 사용하거나, 키프레임을 적용해 루트모션을 구현했습니다.

모션워핑을 이용해 루트 모션의 가중치를 설정하거나, 타깃 위치로 트랜스폼을 변경할 때 애니메이션과 동기화를 위해 사용했습니다.

## 7. AI

`Source/GASToy/AI`

BehaviorTree기반으로 AI를 제작했습니다. 

전투 시작시 인트로 어빌리티 실행, 이후 플레이어 거리에 따라 처리합니다.

크게 3가지 타입으로 구성 : 근거리 스킬, 대쉬 스킬, 원거리 스킬

각 거리에 해당하는 스킬을 여러 개 배치해 순차적으로 실행하여 다양한 전투 구현

주기적으로 플레이어와 거리를 벌려 근접, 원거리 전투를 번갈아가며 경험할 수 있도록 제작했습니다.
